...
1. **Burn tensors + devices**: shapes, dtypes, broadcasting, `Device`, moving tensors between devices.
2. **Backends mental model**: what changes across `NdArray` / `Wgpu` / others, and what shouldn’t.
3. **Autodiff**: `ADBackendDecorator`, gradient flow, “why no grad?” debugging checklist.
4. **Modules**: `#[derive(Module)]`, parameter ownership, save/load, initialization patterns.
5. **Configs**: `#[derive(Config)]`, reproducible hyperparameters, serialization of configs.
6. **Training (high-level)**: dataset → dataloader → model → loss → optimizer → metrics → checkpointing.
7. **Training (low-level)**: write a tiny custom loop to understand what the `Learner` abstracts.
...
14. **Inference & deployment shape**: model packaging, feature parity, deterministic inference, monitoring.

## Phase 2: Burn Core Concepts (The “What”)

### What you must be able to explain (in your own words)

- Why Burn makes `Tensor<B, D>` generic over a `Backend`
- When you need autodiff and when you explicitly do *not* want it (inference speed/footprint)
...
- `Learner` configuration: metrics, checkpoints, early stopping (or your equivalent)
- Optimizers, schedulers, and common failure modes (divergence, NaNs, exploding gradients)

### Deliverables 1

- `docs/notes/burn_training_framework.md`
...
- Reproducibility: deterministic seeds, config snapshots, “what code produced this model”
- Inference contract: feature ordering, scaling, missing data policy
- Monitoring: drift, performance decay, latency/memory budgets
