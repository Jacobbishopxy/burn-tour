...
- [x] 2) Backends mental model ([code](../crates/burn_lab/src/bin/03_backends_mental_model.rs), [note](../docs/notes/burn_backends_mental_model.md)) — Learned that `Backend` is a compile-time choice that fixes `Tensor` storage/kernels and the concrete `Device` type you pass around. Still unclear: which backend wrappers (autodiff/fusion) are “mostly plumbing” vs which change performance/semantics in ways I should measure early.
- [ ] 3) Autodiff debugging checklist ([code](../crates/burn_lab/src/bin/04_autodiff_debugging_checklist.rs), [note](../docs/notes/burn_autodiff_debugging_checklist.md)) — Learned that you need both an autodiff backend (`Autodiff<...>`) and `require_grad()` on leaf tensors to get gradients. The most common “why is grad None?” failure mode so far is accidentally breaking the graph with `detach()`/inner conversions while printing/debugging.
